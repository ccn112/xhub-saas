  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-xl font-semibold text-gray-800 dark:text-dark-50">Cơ hội bán hàng</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-dark-300">
            Pipeline theo giai đoạn (BO-0202). Thắng (WON) không tự tính là doanh thu — chỉ hợp đồng mới tính.
          </p>
        </div>
      ...
      <Card className="p-3">
        <div className="flex flex-wrap items-center gap-2 text-sm">
          <span className="text-gray-500 dark:text-dark-300">Giai đoạn:</span>
          {STAGE_FILTERS.map((s) => (
            <Link
              key={s || "ALL"}
            ...
            >
              {s ? OPPORTUNITY_STAGE_LABEL[s] ?? s : "Tất cả"}
            </Link>
          ))}
        </div>
      </Card>
        ...
        <table className="w-full min-w-[720px] text-sm">
          <thead>
            <tr className="border-b border-gray-200 text-left text-xs text-gray-500 dark:border-dark-600 dark:text-dark-300">
              ...
              <th className="px-3 py-2 font-medium">Giá trị dự kiến</th>
            ...
            {items.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-3 py-6 text-center text-gray-400">
                  {source === "offline" ? "Backend offline." : "Chưa có cơ hội nào khớp bộ lọc."}
                </td>
              </tr>
            ) : null}
          </tbody>
