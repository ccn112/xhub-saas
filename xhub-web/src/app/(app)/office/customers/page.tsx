  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-heading text-xl font-semibold text-gray-800 dark:text-dark-50">{t("customersTitle")}</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-dark-300">{t("customersSubtitle")}</p>
        </div>
      ...
      <Card className="p-3">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
          <span className="text-gray-500 dark:text-dark-300">{t("statusLabel")}</span>
          {STATUS_FILTERS.map((s) => (
            ...
            >
              {s ? CUSTOMER_STATUS_LABEL[s] ?? s : t("allFilter")}
            </Link>
          ))}
        </div>
      </Card>
        ...
        <table className="w-full min-w-[720px] text-sm">
          <thead>
            <tr className="border-b border-gray-200 text-left text-xs text-gray-500 dark:border-dark-600 dark:text-dark-300">
              ...
              <th className="px-3 py-2 font-medium">{t("colIndustry")}</th>
              <th className="px-3 py-2 font-medium">{t("colPrimaryContact")}</th>
            ...
            {items.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-3 py-6 text-center text-gray-400">
                  {source === "offline" ? t("emptyOffline") : t("emptyCustomers")}
                </td>
              </tr>
            ) : null}
          </tbody>
