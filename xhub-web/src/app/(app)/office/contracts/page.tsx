...
export const metadata = { title: "Hợp đồng · X.Office" };
            ...
            {items.map((c) => (
              <tr key={c.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 dark:border-dark-700 dark:hover:bg-dark-800">
                <td className="px-3 py-2">
                  <Link href={`/office/contracts/${c.id}`} className="font-medium text-primary-600 hover:underline dark:text-primary-400">
                    {c.contractNo}
                  </Link>
                </td>
                <td className="px-3 py-2 text-gray-600 dark:text-dark-200">{c.customer?.name ?? "—"}</td>
                <td className="px-3 py-2 text-gray-700 dark:text-dark-100">{formatMoney(c.totalAmount, c.currency)}</td>
                <td className="px-3 py-2 text-gray-600 dark:text-dark-200">{c.effectiveFrom ? new Date(c.effectiveFrom).toLocaleDateString("vi-VN") : "—"}</td>
                <td className="px-3 py-2">
